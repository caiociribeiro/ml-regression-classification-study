...
def load_dataset(dataset_id, target_name):
    ...
    # Extrai numpy array de X de forma defensiva (sem depender de pandas)
    if hasattr(X, "values"):
        X_arr = np.asarray(X.values)
    else:
        X_arr = np.asarray(X)

    # Normaliza arrays 1D/2D
    if X_arr.ndim == 1:
    ...
    # Determina nomes das features correspondentes às colunas em X_arr
    feature_names = None
    if attribute_names:
        try:
